...
- Prepared the package as the standalone PI Team Room extension candidate
  `pi-team-room`, with the public PI entry set to
  `team-runtime/standalone-extension.ts`.
- Rewrote the README for the mobile-first single chat and persistent Team Web
  product instead of the original terminal Messenger workflow.
- The default empty Team now initializes from detected local access methods and
  PI model scope. Existing Teams are no longer auto-expanded, trimmed or
