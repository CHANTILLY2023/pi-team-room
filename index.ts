...
import { TeamRuntimeManager } from "./team-runtime/extension.ts";
...
Usage (action-based API - preferred):
  ...
  // Persistent PI Agent Team Runtime
  pi_messenger({ action: "team.runtime.web" })                   → Open default Web Team with scoped models
  pi_messenger({ action: "team.runtime.setup", name: "core-team" })
  pi_messenger({ action: "team.runtime.agent.add", name: "Alice", model: "openai/gpt-5" })
  pi_messenger({ action: "team.runtime.status" })
  pi_messenger({ action: "team.runtime.send", message: "@all discuss this" })`,
    promptSnippet:
    ...
    parameters: Type.Object({
      ...
      teamId: Type.Optional(Type.String({ description: "Persistent Team ID for team.runtime actions" })),
      threadId: Type.Optional(Type.String({ description: "Canonical Thread ID for team.runtime actions" })),
      threadIds: Type.Optional(Type.Array(Type.String(), { description: "Canonical Thread IDs for team.runtime bulk thread actions" })),
      folderId: Type.Optional(Type.String({ description: "Team Web conversation folder ID for thread categorization" })),
      folderIds: Type.Optional(Type.Array(Type.String(), { description: "Ordered Team Web folder IDs for folder reorder actions" })),
      ...
      clientId: Type.Optional(Type.String({ description: "Runtime client path for a persistent Team Agent: pi, kimi-code, claude-code, codex-cli, grok-build, or grok-pi" })),
      ...
      runtimePolicy: Type.Optional(StringEnum(["always_on", "idle_timeout", "on_demand"], { description: "Persistent Agent runtime residency policy" })),
      ...
      port: Type.Optional(Type.Number({ description: "Localhost port for team.runtime.web (0 or omitted chooses a free port)" })),
      open: Type.Optional(Type.Boolean({ description: "Open the Team Web UI in the browser for team.runtime.web" })),
      memberLimit: Type.Optional(Type.Number({ description: "Maximum scoped models to auto-load into the default Team for team.runtime.web" })),
      ...
      scope: Type.Optional(StringEnum(["team", "thread", "agent_private"], { description: "Durable memory scope" })),
      visibility: Type.Optional(StringEnum(["team", "direct", "private"], { description: "Canonical message or memory visibility" })),
    ...
    async execute(_toolCallId, rawParams, signal, _onUpdate, ctx) {
      ...
      if (action === "team.runtime" || action.startsWith("team.runtime.")) {
        return teamRuntime.handleAction(action, params, ctx, signal);
      }
  ...
  pi.registerCommand("messenger", {
    description: "Open messenger overlay, 'team' for persistent Team view, or 'config' to manage settings",
    handler: async (args, ctx) => {
      ...
      const callbacks: OverlayCallbacks = {
        ...
        getTeamRuntimeSnapshot: () => teamRuntime.getOverlaySnapshot(),
        onTeamMessage: async (message) => {
          const response = await teamRuntime.handleAction("team.runtime.send", { message }, ctx);
          return {
            success: !(response.details as { error?: unknown }).error,
            message: response.content[0]?.text ?? "",
          };
        },
      };
      const initialMode = command === "team" ? "team" : "list";

      const snapshot = await ctx.ui.custom<string | undefined>(
        (tui, theme, _keybindings, done) => {
          overlayTui = tui;
          return new MessengerOverlay(tui, theme, state, dirs, done, callbacks, ctx.cwd, initialMode);
        },
        {
          overlay: true,
          onHandle: (handle) => {
  ...
  // Preserve the established Messenger lifecycle-handler order. The Team
  // runtime is lazy, so registering it last does not open storage or delay
  // startup; it only appends its input/shutdown hooks after existing ones.
